...
function diffuse_bad(N, b, x, x0, diff, dt) {
  let a = dt * diff * N * N;
  for (let i = 1; i <= N; i++) {
    for (let j = 1; j <= N; j++) {
      x[IX(N, i, j)] =
...
function project(N, u, v, p, div) {
  // compute -div of velocity field and initialize pressure field to 0
  for (let i = 1; i <= N; i++) {
    for (let j = 1; j <= N; j++) {
      div[IX(N, i, j)] =
        (-0.5 *
...
class Fluid {
  constructor(N, visc, diff, useDiffuseBad, useDiffuseAdvection, useVelStep) {
    this.N = N;
    this.visc = visc || 0;
    this.diff = diff || 0;
    ...
    this.useDiffuseBad = useDiffuseBad;
    this.useDiffuseAdvection = useDiffuseAdvection;
    this.useVelStep = useVelStep;
  }
